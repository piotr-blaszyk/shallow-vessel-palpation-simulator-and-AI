...
| `sensor_mesh_screenshots.sh` | project page only — six orthogonal Gmsh screenshots of the ViTacTip mesh (`main/sensor_mesh_screenshots.py`), lossless WebP in `docs/images/sensor_mesh/`. Gmsh has no off-screen renderer, so its FLTK window opens briefly; `General.Trackball=0` is required for the Euler-angle views to apply, and "along +x" = line of sight +x (camera on the −x side). Lossless WebP beats lossy here (flat colours + sharp edges: ~20 kB vs ~110 kB) |
