...
| `manuscript_artifacts/` → restored to `difftactile/output/manuscript_artifacts/` | ~8 MB | **The manuscript's figures and tables, as published — the one "generable but included" exception.** `figures/`: the Fig. 5 alignment panels (`alignment_{press,twist_z,twist_x,slide,all}.png`) and the Fig. 6 mean PR curves plus their ROC twins (`mean_{pr,roc}_curve_<config>.pdf`, five-seed mean ± 1 std). `tables/`: `AUROC_RESULTS.md` (per-scenario metrics of the published checkpoints), `sweep.json` (every per-seed metric behind Fig. 6 and the best-of-five choice), `CLIP_LEN_ABLATION.md` (Table 3), `FRAME_SPACE_METRICS.md` (Table 4, upper half), `alignment_validation.json` (the MAEs beside Fig. 5). `vessel_maps/<train>-to-<test>_gt-<source>/`: the latest `vessel_map_all.sh` run of each of the six map configurations (Fig. 7, Table 4 lower half) with `confusion_rNN.{png,pdf}`, `metrics_by_radius.md`, `report.md`, `run.json`. `manuscript_artifacts/README.md` maps every file to its manuscript figure/table and to the repository path and script it came from. |
