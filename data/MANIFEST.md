...
| `analysis/results/frame_space_predictions_<config>.npz` | ~1.5 MB | Per-marker probability, label, pixel position, trial and frame index of every central test frame, for all four published models — stage 1 of `docker/reproduce_analysis.sh`. Regenerable in about a minute, but **only on a GPU**, and every other stage of the poster's analysis reads them; shipping them is what lets a CPU-only machine rebuild every poster figure and number. They are gitignored (`*.npz`), so the bundle is the only place they exist outside a run. Because they are regenerable, `restore_data.sh --verify` reports them as `ABSENT  … (regenerable)` rather than failing, so a bundle downloaded before they were added still verifies. |
...
Note on the poster's analysis (`analysis/`): only the four `.npz` above are shipped.
Everything else that tree produces — the metric JSON, the figures and the Markdown
reports — is regenerated from them in about a minute by `./docker/reproduce_analysis.sh`
and is tracked in git, so shipping it again would duplicate the repository. The published
top-view map runs the analysis also reads are already in the bundle, under
`manuscript_artifacts/vessel_maps/`.
