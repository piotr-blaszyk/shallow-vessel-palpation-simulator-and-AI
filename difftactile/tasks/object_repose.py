...
class Contact:
    def __init__(self, use_state, use_tactile, dt=5e-5, total_steps=100, sub_steps=50,obj=None):
        ...
        # visualization
        self.surf_offset1 = ti.Vector.field(2, float, self.fem_sensor1.num_contact_surface_triangles, needs_grad=True)
        self.surf_init_pos1 = ti.Vector.field(2, float, self.fem_sensor1.num_contact_surface_triangles, needs_grad=True)
        self.press_offset1 = ti.field(float, self.fem_sensor1.num_contact_surface_triangles, needs_grad=True)

        self.draw_pos2 = ti.Vector.field(2, float, self.fem_sensor1.num_vertices, needs_grad=True) # elastomer1's pos
        ...
        self.draw_fem1_3d = ti.Vector.field(3, dtype=float, shape=(self.fem_sensor1.num_vertices), needs_grad=True)
        ...
        self.color_fem1_3d = ti.Vector.field(3, dtype=float, shape=(self.fem_sensor1.num_vertices), needs_grad=True)
    ...
    def compute_contact_force(self, f:ti.i32):
        for i in range(self.fem_sensor1.num_contact_surface_triangles):
            ...
            self.contact_force1[None] += 1/6 * self.fem_sensor1.contact_forces_on_vertices[f,a] 
            self.contact_force1[None] += 1/6 * self.fem_sensor1.contact_forces_on_vertices[f,b] 
            self.contact_force1[None] += 1/6 * self.fem_sensor1.contact_forces_on_vertices[f,c] 

    @ti.kernel
    def compute_force_loss(self, t:ti.i32):
        self.predict_force1[t] = self.fem_sensor1.inverse_rotation_matrix[None] @ self.contact_force1[None]
    ...
    def compute_marker_loss_1(self, f: ti.i32):
        for i in range(self.fem_sensor1.num_markers):
            sim_marker = self.fem_sensor1.deformed_markers[i]
    ...
    def apply_action(self, action, ts):
        ...
        self.fem_sensor1.local_translational_velocity.from_numpy(d_pos1)
        self.fem_sensor1.local_angular_velocity.from_numpy(d_ori1)
    ...
    for opts in range(0, num_opt_steps):
        ...
        for ts in range(num_total_steps-1):
            ...
            if ts == 50:
                ...
                print(contact_model.fem_sensor1.deformed_markers.grad[0])
                print(contact_model.fem_sensor1.vertex_positions_deformed.grad[0, 0])
                print(contact_model.fem_sensor1.vertex_velocities.grad[0, 0])
