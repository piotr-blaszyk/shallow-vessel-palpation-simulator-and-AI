...
7. **Reproduce the poster's analysis** (`./reproduce_analysis.sh`) → [Which script produces
   which figure or table on the poster](#which-script-produces-which-figure-or-table-on-the-poster).
...
### Which script produces which figure or table on the poster

The conference **poster** reports more than the manuscript: the same four models scored as a
*detection* problem (counts, Hungarian matching, F1 and OSPA), a per-vessel
centreline/centroid decomposition, and a baseline comparison in which **our own predictions
are recomputed under each baseline's own metric definition** rather than being printed beside
numbers that measure a different quantity. All of it lives in
[`analysis/`](analysis/) and is regenerated by one wrapper:

```bash
./docker/reproduce_analysis.sh                  # stages 1-12: every poster figure and number
./docker/reproduce_analysis.sh --with-overlays  # + per-data-point diagnostic overlays (~15 min)
./docker/reproduce_analysis.sh --list           # the stage table
./docker/reproduce_analysis.sh --only 4,7       # re-run individual stages
```

It trains nothing and simulates nothing: it evaluates the four published checkpoints once,
...
|---|---|---|
| **Methods** — *Workflow*: sensor, mesh, simulated interactions, domain randomisation, real datasets, annotation | `figures/workflow/{vitactip,mesh,A1,A2,B1,B2,B3,dr_*,real_*,ann_*}.png` | `make_workflow_images.py` (12) |
...
| **Results** — take-home box (1.2 mm; AP 0.32 vs 0.30) | `results/symmetric_distances.json`, `figures/pr/ap_values.json` | `symmetric_distances.py` (2), `pr_curves_with_ap.py` (8) |
...
| **Results** — *Precision–recall curves* | `figures/pr/mean_pr_curve_<config>.pdf`, `curve_legend_vertical.pdf`, `xlabel_recall.pdf` | `pr_curves_with_ap.py` (8) |
| **Results** — *Foreground IoU* | `figures/segmentation_iou.pdf` | `symmetric_distances.py` (2) → `segmentation_bars.py` (6) |

Three poster blocks are **not** repository output and have no script: the Motivation
illustration (hand-made), the *Vessel-detection modalities* table (literature values; the
fully cited version is published at
[`docs/poster-motivation-bibliography/`](docs/poster-motivation-bibliography/)), and the
baselines' own half of the comparison table (read out of each cited paper's full text).

[`analysis/README.md`](analysis/README.md) carries the per-script input/output table, the
self-checks the pipeline performs, the parameters that are judgements rather than fits
(the 5 mm OSPA cutoff, the 2:1 false-alarm/miss weighting, the 3 mm peak-splitting rule)
and the caveats that belong beside every number.
...
marker tracks, the published domain-adaptation run, the poster analysis's per-marker
predictions (`analysis/results/frame_space_predictions_<config>.npz` — one minute of GPU time,
shipped so stages 2–12 of `./docker/reproduce_analysis.sh` need no GPU at all), and
`difftactile/output/manuscript_artifacts/` (the manuscript's figures and tables, with a README
mapping each to its source — the one "generable but shipped" exception; its `vessel_maps/`
copy is also what the poster analysis reads when the repository has not run the maps itself). Raw videos, intermediate preprocessing stages, superseded
...
analysis/                    the poster's analysis: scripts/ (detection + OSPA scoring,
                             centreline decomposition, baseline-comparable metrics, every
                             poster figure), results/, figures/, reports/
...
the poster's own results are under `analysis/results/` and `analysis/reports/`.
