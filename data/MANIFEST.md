...
| `difftactile/manual_or_experimental_data/meat_training_data/clean/*/frames.mp4` | ~52 MB | The 26 camera frames per meat trial that preprocessing kept, H.264 CRF 26 (~2.3 MB each). Frame *i* corresponds 1:1 with row *i* of `marker_labels.npz`, so the annotation viewer can draw the ground-truth labels over the real meat images (paper Fig. `annotation-line`(d)). Cut down from the 1.6 GB raw recordings by `script_make_meat_clean_videos`; the raw videos themselves are a physical recording and are not shipped. |
