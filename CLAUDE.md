...
**The published simulated dataset predates this and cannot be regenerated by any seed.** A
default collection run now produces a different draw, not the shipped one — restore that from
the Zenodo bundle rather than expecting `DIFFTACTILE_SEED=42` to rebuild it.
