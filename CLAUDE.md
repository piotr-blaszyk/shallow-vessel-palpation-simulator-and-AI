...
| `record_domain_randomisation_videos.sh` | project page only — seven vessel-present slides, one per domain-randomisation configuration (heading −15/0/+15° from a TOP-DOWN camera, `DIFFTACTILE_CAMERA_VIEW=top`; sensor↔vessel stiffness 5e3/5e4 and damping 0/100 from the side view; the other two parameters at their midpoint 0° / 27 500 / 50). `videos/dr_*.mp4` + `docs/videos/`. The pins `DIFFTACTILE_SLIDE_HEADING_DEG` (read in `get_slide_trajectory()`, skips the random draw), `DIFFTACTILE_VEIN_NORMAL_STIFFNESS` / `_DAMPING` (applied in `record_da_trajectories_main()` only) exist for this; never set them for dataset collection. |
